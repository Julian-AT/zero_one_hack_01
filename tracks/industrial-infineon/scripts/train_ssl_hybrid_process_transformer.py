#!/usr/bin/env python3
"""
Hybrid self-supervised Process Transformer.

This model keeps full process steps as target tokens but enriches the input
with semantic process features:
    ...
    + block embedding
    + side embedding
    + litho-level embedding

Input CSV:
    SEQUENCE_ID,FAMILY,STEP

Example:
    python tracks/industrial-infineon/scripts/train_ssl_hybrid_process_transformer.py \
      --data tracks/industrial-infineon/data/generated/valid_long_augmented.csv \
      --out-dir tracks/industrial-infineon/runs/ssl_hybrid_augmented_smoke \
      --max-sequences 3000 \
      --epochs 2 \
      --batch-size 64 \
      --d-model 128 \
...
def load_official_sets():
    """
    Load validator vocabulary sets from training_data/generate_sequences.py.
    If loading fails, fall back to empty sets and rely on string parsing.
    """
    root = Path(__file__).resolve().parents[1]
    gen_path = root / "training_data" / "generate_sequences.py"

    if not gen_path.exists():
        return {}

    spec = importlib.util.spec_from_file_location("generate_sequences", gen_path)
    ...
    out = {}
    for name in names:
        out[name] = set(getattr(mod, name, set()))
    return out


OFFICIAL = load_official_sets()
...
def parse_level(step: str) -> str:
    if is_special(step):
        return "SPECIAL"

    m = re.search(r"\bLEVEL\s+(\d+)\b", step)
    if m:
        return f"LEVEL_{m.group(1)}"
    return "NO_LEVEL"


def parse_side(step: str) -> str:
    if is_special(step):
        return "SPECIAL"
    if "BACKSIDE" in step:
        return "BACKSIDE"
    if "FRONTSIDE" in step:
        return "FRONTSIDE"
    if "WAFER_EDGE" in step or "EDGE" in step:
        return "EDGE"
    return "GENERAL"
...
def parse_action(step: str) -> str:
    ...
    if s.startswith("MEASURE") or " MEASURE" in s:
        return "MEASURE"
    if "INSPECTION" in s or s.startswith("INSPECT") or "CHECK" in s:
        return "INSPECT_CHECK"
    if s.startswith("DEPOSIT"):
        return "DEPOSIT"
    if "ETCH" in s and "ETCH CLEAN" not in s:
        return "ETCH"
    if "CLEAN" in s or s in {"HF DIP", "OXIDE STRIP"} or "RINSE" in s or "DRY" in s:
        return "CLEAN"
    if s.startswith("SPIN COAT"):
        return "COAT"
    if "BAKE" in s:
        return "BAKE"
    ...
    if s.startswith("EXPOSE"):
        return "EXPOSE"
    if s.startswith("DEVELOP"):
        return "DEVELOP"
    if s.startswith("STRIP"):
        return "STRIP"
    if s.startswith("IMPLANT"):
        return "IMPLANT"
    if "ANNEAL" in s or "DIFFUSION" in s or "DENSIFY" in s:
        return "THERMAL"
    if s.startswith("CMP"):
        return "CMP"
    ...
    if "TEST" in s:
        return "TEST"
    if s.startswith("RECEIVE") or s.startswith("LOT") or s.startswith("SHIP"):
        return "LOGISTICS"
    if "GRIND" in s or "GRINDING" in s:
        return "GRIND"
    if "PREP" in s:
        return "PREP"

    return s.split()[0] if s.split() else "OTHER"


def parse_role(step: str) -> str:
    if is_special(step):
        return "SPECIAL"

    dep = OFFICIAL.get("DEPOSITION_STEPS", set())
    clean = OFFICIAL.get("CLEAN_STEPS", set())
    etch = OFFICIAL.get("ETCH_STEPS", set())
    metal_etch = OFFICIAL.get("METAL_ETCH_STEPS", set())
    implant = OFFICIAL.get("IMPLANT_STEPS", set())
    cmp_steps = OFFICIAL.get("CMP_STEPS", set())
    pad = OFFICIAL.get("PAD_WINDOW_STEPS", set())
    tests = OFFICIAL.get("ELECTRICAL_TEST_STEPS", set())
    backside_metal = OFFICIAL.get("BACKSIDE_METAL_STEPS", set())
    ...
    if s in pad:
        return "PAD_WINDOW"
    if s in tests:
        return "ELECTRICAL_TEST"
    if s in backside_metal:
        return "BACKSIDE_METAL"
    if s in dep:
        return "DEPOSITION"
    if s in clean:
        return "CLEAN"
    if (
        s.startswith("ALIGN")
        or s.startswith("EXPOSE")
        or s.startswith("DEVELOP")
        or s.startswith("SPIN COAT")
        or "BAKE" in s
    ):
        return "LITHOGRAPHY"
    if s.startswith("MEASURE") or "INSPECTION" in s or "CHECK" in s:
        return "METROLOGY"
    if "ANNEAL" in s or "DIFFUSION" in s or "DENSIFY" in s:
        return "THERMAL"
    if s.startswith("FILL"):
...
def parse_material(step: str) -> str:
    if is_special(step):
        return "SPECIAL"
    ...
    checks = [
        ("PHOTORESIST", ["PHOTORESIST", "RESIST"]),
        ("POLYSILICON", ["POLYSILICON", "POLY"]),
        ("TUNGSTEN", ["TUNGSTEN"]),
        ("METAL", ["METAL", "BARRIER", "SEED", "CONTACT"]),
        ("PASSIVATION", ["PASSIVATION"]),
        ("DIELECTRIC", ["DIELECTRIC", "INTERLAYER", "INTERLEVEL", "ILD"]),
        ("OXIDE", ["OXIDE"]),
        ("EPITAXY", ["EPITAXY", "EPITAXIAL"]),
        ("VIA", ["VIA"]),
        ("PAD", ["PAD"]),
        ("GATE", ["GATE"]),
        ("SOURCE_DRAIN", ["SOURCE", "DRAIN", "CATHODE"]),
        ("BODY_BUFFER_CHANNEL", ["BODY", "BUFFER", "CHANNEL", "LDD"]),
        ("JUNCTION", ["JUNCTION"]),
...
def parse_block(step: str) -> str:
    ...
    if "FINAL" in s:
        return "FINAL"
    if "TEST" in s or "YIELD" in s:
        return "TEST_SORT"
    if "BACKSIDE" in s or "GRIND" in s:
        return "BACKSIDE"
    if "PASSIVATION" in s or "PAD" in s:
        return "PASSIVATION_PAD"
    if "METAL" in s or "TUNGSTEN" in s or "CONTACT" in s:
        return "METALLIZATION"
    if "VIA" in s:
        return "VIA_CONTACT"
    if "DIELECTRIC" in s or "INTERLAYER" in s or "INTERLEVEL" in s or "CMP" in s:
        return "ILD_CMP"
    if "POLY" in s or "GATE" in s:
        return "GATE_STACK"
    if "IMPLANT" in s or "JUNCTION" in s or "DIFFUSION" in s:
        return "IMPLANT_DIFFUSION"
    if "LITHO" in s or "MASK" in s or "PHOTORESIST" in s or "BAKE" in s or "DEVELOP" in s:
        return "LITHOGRAPHY"
    if "ETCH" in s:
        return "ETCH"
    if "OXIDE" in s or "OXIDATION" in s:
        return "OXIDATION"
    if "EPITAX" in s or "SUBSTRATE" in s:
        return "EPITAXY_SUBSTRATE"
    if "CLEAN" in s or s in {"HF DIP", "OXIDE STRIP"} or "RINSE" in s or "DRY" in s:
        return "CLEANING"
    if s.startswith("MEASURE") or "INSPECTION" in s or "CHECK" in s:
        return "METROLOGY"
...
def feature_dict_for_token(token: str):
    token = normalize_step(token)

    return {
        "action": parse_action(token),
        "role": parse_role(token),
        "material": parse_material(token),
        "block": parse_block(token),
        "side": parse_side(token),
        "level": parse_level(token),
    }


def load_sequences(path: Path, max_sequences: int, seed: int):
    df = pd.read_csv(path)
    ...
    for sid, g in df.groupby("SEQUENCE_ID", sort=False):
        family = normalize_step(g["FAMILY"].iloc[0])
        steps = [normalize_step(x) for x in g["STEP"].tolist()]
        if len(steps) >= 2:
...
def split_records(records, train_frac, val_frac, seed):
    ...
    train, val, test = [], [], []
    for _, rs in sorted(by_family.items()):
        rng.shuffle(rs)
        n = len(rs)
...
def build_vocabs(records):
    ...
    feature_values = {name: {"UNKNOWN", "SPECIAL"} for name in FEATURE_NAMES}
    ...
    for name in FEATURE_NAMES:
        vals = sorted(feature_values[name])
        feature_to_id[name] = {v: i for i, v in enumerate(vals)}

    token_feature_table = torch.zeros((len(token_to_id), len(FEATURE_NAMES)), dtype=torch.long)
...
class ProcessDataset(Dataset):
    def __init__(self, records, token_to_id, family_to_id, max_len):
...
class HybridCollator:
    def __init__(
        ...
        family_unknown_id,
        family_dropout,
        input_step_mask_prob,
        train,
    ):
        self.pad_id = pad_id
        self.bos_id = bos_id
        self.mask_id = mask_id
        self.token_feature_table = token_feature_table
        self.unknown_feature_ids = unknown_feature_ids
    ...
    def __call__(self, batch):
        ...
        feature_ids = self.token_feature_table[input_ids].clone()
        ...
        if self.train and self.input_step_mask_prob > 0:
            ...
            for j in range(feature_ids.size(-1)):
                feature_ids[:, :, j][mask] = self.unknown_feature_ids[j]
...
class HybridProcessTransformerLM(nn.Module):
    def __init__(
        self,
        vocab_size,
        num_families,
        ...
        self.lm_head.weight = self.step_emb.weight

        self.dropout = nn.Dropout(dropout)

    def forward(self, input_ids, feature_ids, family_ids):
        bsz, seqlen = input_ids.shape
        device = input_ids.device

        pos = torch.arange(seqlen, device=device).unsqueeze(0).expand(bsz, seqlen)

        x = self.step_emb(input_ids)
        x = x + self.family_emb(family_ids).unsqueeze(1)
        x = x + self.pos_emb(pos)

        for j, emb in enumerate(self.feature_embs):
            x = x + emb(feature_ids[:, :, j])
...
def make_class_weights(records, token_to_id, max_len, mode):
...
def lr_schedule(step, base_lr, warmup_steps, total_steps):
...
def evaluate(model, loader, criterion, device, pad_id):
    ...
    for batch in loader:
        ...
        feature_ids = batch["feature_ids"].to(device, non_blocking=True)
        target_ids = batch["target_ids"].to(device, non_blocking=True)
        family_ids = batch["family_ids"].to(device, non_blocking=True)

        logits = model(input_ids, feature_ids, family_ids)
...
def main():
    ...
    ap.add_argument("--family-dropout", type=float, default=0.25)
    ap.add_argument("--input-step-mask-prob", type=float, default=0.03)
    ap.add_argument("--class-weight", choices=["none", "inverse_sqrt"], default="none")
    ...
    print(f"Using data file: {cfg.data}")

    records = load_sequences(Path(cfg.data), cfg.max_sequences, cfg.seed)
    train_records, val_records, test_records = split_records(
        records, cfg.train_frac, cfg.val_frac, cfg.seed
    )
    ...
    feature_vocab_sizes = [len(feature_to_id[name]) for name in FEATURE_NAMES]
    ...
    train_collator = HybridCollator(
        pad_id=pad_id,
        bos_id=bos_id,
        mask_id=mask_id,
        token_feature_table=token_feature_table,
    ...
    print("[4/6] Building hybrid model...")
    if cfg.device == "auto":
        device = "cuda" if torch.cuda.is_available() else "cpu"
    else:
        device = cfg.device

    model = HybridProcessTransformerLM(
        vocab_size=len(token_to_id),
