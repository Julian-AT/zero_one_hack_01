...
> code in `models/neurosymbolic/`, run the CPU experiments locally, then use
...
```
                ...
                competition/track-details/training_data/generate_sequences.py
                                       │ import by path
                                       ▼
...
Every NSPE module lives in `models/neurosymbolic/`. The **only** repo
...
`competition/track-details/scripts/generate_ood_families.py` already does.
...
| Asset | Path | Used for |
|---|---|---|
| Official validator + grammar | `competition/track-details/training_data/generate_sequences.py` | `validate_sequence`, `generate_sequence`, `read_csv_sequences` |
| Official scorer | `competition/track-details/scripts/eval_metrics.py` | scoring all 3 tasks (run via subprocess) |
| Training sequences (long fmt) | `competition/track-details/training_data/{MOSFET,IGBT,IC}_variants.csv` | 1000 seqs/family |
| Fab parameters (optional) | `competition/track-details/training_data/{MOSFET,IGBT,IC}_longdescription_parameters.csv` | optional role-induction fallback |
| Sample eval inputs | `competition/track-details/scripts/eval_input_valid.csv` (600) · `eval_input_anomaly.csv` (987) | dry-run the submission pipeline |
| OOD family generator | `competition/track-details/scripts/generate_ood_families.py` | simulate the unseen 4th family for OOD tests |
| Env (CUDA 12.1 torch) | `pixi.toml` (repo root) | GPU runs on Leonardo via `pixi run` |

**Self-containment rule:** all *new* code goes in `models/neurosymbolic/`. The
...
```bash
cd /Users/kyrill/IdeaProjects/zero_one_hack_01/models/neurosymbolic
...
TRACK = REPO / "competition" / "track-details"
...
from the repo root (`PYTHONPATH=models/neurosymbolic`).
...
pixi run python models/neurosymbolic/experiments/ood_symbolic_probe.py
pixi run python models/neurosymbolic/experiments/exp01_symbolic_anomaly.py
pixi run python models/neurosymbolic/experiments/exp03_neural_ranker.py --config models/neurosymbolic/configs/small.yaml --max-steps 200 --smoke
```
...
CUDA_VISIBLE_DEVICES=0 pixi run python $E/exp03_neural_ranker.py --holdout mosfet --config models/neurosymbolic/configs/base.yaml &
CUDA_VISIBLE_DEVICES=1 pixi run python $E/exp03_neural_ranker.py --holdout igbt   --config models/neurosymbolic/configs/base.yaml &
CUDA_VISIBLE_DEVICES=2 pixi run python $E/exp03_neural_ranker.py --holdout ic     --config models/neurosymbolic/configs/base.yaml &
CUDA_VISIBLE_DEVICES=3 pixi run python $E/exp04_constraint_loss.py --config models/neurosymbolic/configs/base.yaml &
wait
...
```bash
sbatch models/neurosymbolic/slurm/train_full.sbatch   # trains on ALL 3 families, runs exp06
# retrieve:
