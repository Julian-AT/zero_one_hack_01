...
Place this file in:
    competition/track-details/data/generate_hard_invalid_sequences.py
...
try:
    ...
    raise RuntimeError(
        ...
        "Make sure generate_sequences.py is in competition/track-details/training_data/."
    ) from exc
...
try:
    ...
    raise RuntimeError(
        ...
        "Make sure coverage_tracker.py is in competition/track-details/data/."
    ) from exc
...
def hard_RULE_METAL_ETCH_NO_LITHO(
    ...
    index = choose_index(
        ...
        lambda i, s: (
            ...
            and bool(
                previous_indices_within_predicate(
                    steps,
                    i,
...
def hard_RULE_PAD_OPEN_BEFORE_DEP(
    ...
    pad_indices = [i for i, s in enumerate(steps) if s in PAD_WINDOW_STEPS and i > cure_idx]
    if not pad_indices:
        return None
...
def hard_RULE_TEST_BEFORE_PASSIVATION(
    ...
    test_indices = [i for i, s in enumerate(steps) if s in ELECTRICAL_TEST_STEPS and i > cure_idx]
    if not test_indices:
        return None
...
HARD_MUTATORS: dict[str, Callable[[list[str], str, random.Random, int], MutationResult | None]] = {
...
def main() -> None:
    ...
    print("\nNext step:")
    print(
        "  Inspect hard_invalid_generation_stats.csv and coverage_report_mixed/coverage_report.md."
    )
