...
comparable scores via the organizer scorer `competition/participant-files/eval_metrics.py`.
...
|---|---|---|
| Trigram-with-backoff | n-gram baseline (memorization floor) | `shared/extras/baselines/trigram_baseline.py` |
| Retrieval | nearest-neighbor baseline | `shared/extras/baselines/retrieval_baseline.py` |
| **SSL Transformer (hybrid, coverage-guided)** | Transformer arch | checkpoint `competition/track-details/runs/ssl_hybrid_new_coverage_guided_v1/checkpoint_best.pt` |
| **Neurosymbolic (PPM + constrained neural ranker)** | Neurosymbolic arch | branch `neurosymbolic-model`, `models/neurosymbolic/nspe/` + `outputs/` |
...
> family (recurrent LSTM/xLSTM) is **implemented but untrained** (`models/transformer_xlstm/model/xlstm_model.py`,
...
- **OOD = Leave-One-Family-Out (LoFO):** the track's hidden-4th-family proxy. **True LoFO requires
  ...
  - LoFO is reported **only for models that already have per-fold results** — the trigram baseline
    (`shared/extras/results/baselines/trigram_metrics.md`) and the neurosymbolic ranker
    (`models/neurosymbolic/outputs/exp03_*.json`).
...
python competition/track-details/training_data/generate_sequences.py --family mosfet --count 200 --seed 90001 --output shared/benchmark/_raw/valid_mosfet.csv
python competition/track-details/training_data/generate_sequences.py --family igbt   --count 200 --seed 90002 --output shared/benchmark/_raw/valid_igbt.csv
python competition/track-details/training_data/generate_sequences.py --family ic     --count 200 --seed 90003 --output shared/benchmark/_raw/valid_ic.csv

# 2. fresh invalid sequences (easy + hard) for the anomaly task
python competition/track-details/data/generate_invalid_sequences.py      --seed 90010 --output-dir shared/benchmark/_raw/easy_invalid
python competition/track-details/data/generate_hard_invalid_sequences.py --seed 90011 --output-dir shared/benchmark/_raw/hard_invalid
...
python competition/track-details/data/build_task_datasets.py \
  --valid-input shared/benchmark/_raw/valid_*.csv \
  --easy-invalid-input shared/benchmark/_raw/easy_invalid/invalid_sequences.csv \
  --hard-invalid-input shared/benchmark/_raw/hard_invalid/hard_invalid_sequences.csv \
  --output-dir shared/benchmark/eval_set_v1
```

`shared/benchmark/eval_set_v1/` then holds the labeled ground truth used as the common eval set; a thin
...
`competition/track-details/data/task_datasets_v1/` (held out for the Transformer; unseen by
...
All scores come from `competition/participant-files/eval_metrics.py`. Metrics are tiered: **Primary** =
...
## 5. Leaderboard schema (`shared/benchmark/RESULTS.md`)
...
|---|---|---|---|
| Original provided sequences | `competition/track-details/training_data/{MOSFET,IGBT,IC}_variants.csv`, `synthetic*.csv` | ✅ committed | in repo |
| Rule generator + validator | `competition/track-details/training_data/generate_sequences.py` | ✅ committed | in repo |
| Coverage-guided valid data | `competition/track-details/data/coverage_guided_v1/coverage_guided_sequences.csv` | ❌ gitignored | regenerate via `data/generate_coverage_guided.py` (Leonardo) |
| Easy invalid data | `competition/track-details/data/easy_invalid_v1/invalid_sequences.csv` | ❌ gitignored | regenerate via `data/generate_invalid_sequences.py` |
| Hard invalid data | `competition/track-details/data/hard_invalid_v1/hard_invalid_sequences.csv` | ❌ gitignored | regenerate via `data/generate_hard_invalid_sequences.py` |
| Labeled task datasets | `competition/track-details/data/task_datasets_v1/{next_step_prediction,sequence_completion,anomaly_detection,rule_attribution}.csv` | ❌ gitignored | regenerate via `data/build_task_datasets.py` |
| **Common benchmark eval set** | `shared/benchmark/eval_set_v1/` | ❌ generate | §3 commands (benchmark-only seed) |
| SSL Transformer checkpoint | `competition/track-details/runs/ssl_hybrid_new_coverage_guided_v1/checkpoint_best.pt` + `vocab.json` | ❌ gitignored | on Leonardo; produced by `shared/scripts/run_train_ssl_hybrid_newdata_normal_gpu.slurm` |
| Neurosymbolic model + outputs | `models/neurosymbolic/` (branch `neurosymbolic-model`): `nspe/`, `outputs/exp03_*.json`, `outputs/submission_task*.csv` | ✅ on that branch | `git checkout neurosymbolic-model` or `git worktree add` |
| Trigram / retrieval baselines | `shared/extras/baselines/*.py`; results `shared/extras/results/baselines/` | ✅ committed | in repo (trigram LoFO already computed) |
| Official scorer | `competition/participant-files/eval_metrics.py` | ✅ committed | in repo |
| Organizer eval inputs (unlabeled) | `competition/participant-files/eval_input_valid.csv`, `eval_input_anomaly.csv` | ✅ committed | reference only — cannot be scored locally |
...
3. Score all three tasks with `eval_metrics.py`; collect into `shared/benchmark/results.csv`.
4. Fill LoFO columns from existing trigram + neurosymbolic per-fold outputs; mark Transformer LoFO
   as out-of-scope.
5. Render `shared/benchmark/RESULTS.md` + plots; link from `REPORT.md`.
