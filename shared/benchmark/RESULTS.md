...
>   set** via `competition/participant-files/eval_metrics.py`. Run `shared/benchmark/run_benchmark.py` (see
>   `shared/benchmark/README.md`) on Leonardo to fill the Stage-2 tables.
>
...
|---|---|---|
| SSL Transformer (coverage-guided hybrid) | `models/self-supervised/hybrid_coverage_guided_metrics.csv`, `models/self-supervised/learned_reranker_report.md` | coverage-guided internal test split (large) |
| Trigram-with-backoff | `shared/extras/results/baselines/trigram_metrics.json` | 3000 base seqs, 80/20, seed 42 |
| Grammar decoder | `shared/extras/results/baselines/grammar_decoder_metrics.json` | same base-seq split |
| Retrieval | `shared/extras/results/baselines/retrieval_metrics.json` | same base-seq split |
| Neurosymbolic — neural ranker (~0.68M) | `models/neurosymbolic/outputs/exp03_*.json` (branch `neurosymbolic-model`) | 80 seqs/family LoFO folds |
| Neurosymbolic — PPM (symbolic) | `models/neurosymbolic/outputs/exp03_*.json` | same |
| Neurosymbolic — anomaly oracle | `models/neurosymbolic/outputs/exp01.json` | 600 ID / 600 OOD |
...
| SSL Transformer (cov-guided) | ~4–8M (unverified) | 30 epochs GPU | see `models/self-supervised/README.md` §6/§8 |
...
See `shared/benchmark/README.md`. In short: `prepare_eval_inputs.py` builds one common labeled eval set +
