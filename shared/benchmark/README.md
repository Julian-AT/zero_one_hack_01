...
prepare_eval_inputs.py   →  common eval set (inputs/ + ground_truth/)
  → each model runs on inputs/  →  shared/benchmark/predictions/<model>/predictions_*.csv
...
```bash
source competition/track-details/.venv/bin/activate   # or the repo pixi env
...
```bash
python shared/benchmark/prepare_eval_inputs.py \
  --task-dir competition/track-details/data/task_datasets_v1 \
  --split test \
  --out-dir shared/benchmark/eval_set_v1
```
...
This writes:
- `shared/benchmark/eval_set_v1/inputs/{nextstep,completion,anomaly}_input.csv` — feed to every model
- `shared/benchmark/eval_set_v1/ground_truth/{nextstep,completion,anomaly}_gt.csv` — for scoring

## 2. Produce predictions per model (organizer format)

Put each model's outputs in `shared/benchmark/predictions/<model>/` named exactly
...
Reuse `competition/participant-files/make_eval_predictions.py`, pointing its input/output constants (lines
~38–44) at the benchmark eval set:
```
EVAL_VALID   = ROOT/"shared/benchmark/eval_set_v1/inputs/nextstep_input.csv"     # (also used for completion)
EVAL_ANOMALY = ROOT/"shared/benchmark/eval_set_v1/inputs/anomaly_input.csv"
OUT_DIR      = ROOT/"shared/benchmark/predictions/ssl_transformer"
```
Then `python competition/participant-files/make_eval_predictions.py`. (For next-step + completion the script
...
# then follow models/neurosymbolic/RUN_ON_LEONARDO.md to run its submission generator
# (experiments/exp06_make_submission.py / nspe/official.py) on the benchmark inputs,
# producing task1/2/3 CSVs; copy/rename into shared/benchmark/predictions/neurosymbolic/:
...
Run `shared/extras/baselines/trigram_baseline.py` and `retrieval_baseline.py` to emit organizer-format
predictions on `shared/benchmark/eval_set_v1/inputs/`, into `shared/benchmark/predictions/trigram/` and
`.../retrieval/`. (These scripts already compute the baseline numbers in
`shared/extras/results/baselines/`; point them at the benchmark inputs to get common-split predictions.)

## 3. Score everything and build the leaderboard

```bash
python shared/benchmark/run_benchmark.py \
  --pred-root shared/benchmark/predictions \
  --gt-dir   shared/benchmark/eval_set_v1/ground_truth \
  --out      benchmark
```

Outputs `shared/benchmark/results.csv` and `shared/benchmark/RESULTS_auto.md`. Fold the auto tables into the
Stage-2 section of `RESULTS.md`.

Score a single model/task ad hoc:
```bash
python shared/benchmark/score.py --task next-step \
  --predictions shared/benchmark/predictions/ssl_transformer/predictions_nextstep.csv \
  --ground-truth shared/benchmark/eval_set_v1/ground_truth/nextstep_gt.csv
```
...
- **% rule-valid completions:** `score.py` accepts an optional `validator` callable; wire in the
  validator from `competition/track-details/training_data/generate_sequences.py` to populate it.
