...
OUT = REPO_ROOT / "shared" / "extras" / "plots" / "report"
OUT.mkdir(parents=True, exist_ok=True)


def _read_metrics(cell: str) -> dict | None:
    p = REPO_ROOT / "shared" / "extras" / "results" / "eval" / cell / "metrics.json"
...
def plot_submission_quality(out_path: Path) -> None:
    ...
    ax = axes[2]
    rule_csv = REPO_ROOT / "shared" / "extras" / "results" / "submission_v2_real" / "anomaly.csv"
    if rule_csv.exists():
