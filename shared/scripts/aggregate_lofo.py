...
Crawls:
  shared/extras/checkpoints/<cell_id>/summary.json   — training-side stats (params, loss, wall)
  shared/extras/results/eval/<cell_id>/metrics.json  — eval-side per-family metrics

Joins on cell id (which encodes arch, size, heads, family_dropout, fold).
Emits:
  shared/extras/results/lofo_ablation.csv  — one row per cell, machine-readable
  shared/extras/results/lofo_ablation.md   — Markdown table sorted by held-out Top-1
...
CHECKPOINTS_DIR = REPO_ROOT / "shared" / "extras" / "checkpoints"
EVAL_DIR = REPO_ROOT / "shared" / "extras" / "results" / "eval"
OUT_DIR = REPO_ROOT / "shared" / "extras" / "results"
