...
#   pull-light     – rsync shared/extras/logs + shared/extras/results (no .pt) from $SCRATCH
#   pull           – full rsync of shared/extras/ including checkpoints
#
# Usage:
#   bash shared/scripts/leonardo/deploy.sh probe
#   bash shared/scripts/leonardo/deploy.sh bootstrap
#   bash shared/scripts/leonardo/deploy.sh smoke
#   bash shared/scripts/leonardo/deploy.sh grid
#
...
bootstrap_cmd() {
    ...
    rsync -az --delete \
          ...
          --exclude='__pycache__/' \
          --exclude='.DS_Store' \
          --exclude='shared/extras/checkpoints/' \
          --exclude='shared/extras/logs/' \
...
smoke_cmd() {
    ...
    leo 'cd "$SCRATCH/zero_one_hack_01"
         sbatch --time=00:15:00 \
                ...
                --output=shared/extras/logs/slurm-smoke-%j.out \
                --error=shared/extras/logs/slurm-smoke-%j.err \
                shared/scripts/slurm/train.sbatch
         squeue --me
        '
}
...
grid_cmd() {
    ...
    leo 'cd "$SCRATCH/zero_one_hack_01" && sbatch shared/scripts/slurm/grid.sbatch && squeue --me'
}
...
tail_cmd() {
    ...
    leo "cd \"\$SCRATCH/zero_one_hack_01\" && tail -n 200 shared/extras/logs/slurm-${jobid}.out 2>/dev/null || tail -n 200 shared/extras/logs/grid-*_${jobid}.out 2>/dev/null"
}
...
pull_light_cmd() {
    ...
    rsync -avz \
          ...
          --include='shared/extras/logs/***' \
          --include='shared/extras/results/***' \
          --include='shared/extras/checkpoints/' \
          --include='shared/extras/checkpoints/*/' \
          --include='shared/extras/checkpoints/*/summary.json' \
          --include='shared/extras/checkpoints/*/*.txt' \
          --exclude='*.pt' \
          --exclude='*.safetensors' \
          --exclude='*.bin' \
          --exclude='*' \
...
pull_cmd() {
    ...
    remote_scratch="$(_remote_scratch)"
    local remote_dir="${remote_scratch}/zero_one_hack_01"
    echo "[leo] pulling shared/extras/ (logs + results + checkpoints) → ${REPO_ROOT}/extras/"
