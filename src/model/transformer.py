...
def make_rope_cache(seq_len: int, head_dim: int, device: torch.device,
...
class DecoderTransformer(nn.Module):
    ...
    def _rope(self, device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
        if self._rope_cache is None or self._rope_cache[0].device != device:
            self._rope_cache = make_rope_cache(self.cfg.max_seq_len,
...
def build_transformer(model_cfg: dict, vocab_size: int,
                       enable_multitask_heads: bool = False) -> DecoderTransformer:
    cfg = TransformerConfig(vocab_size=vocab_size, **{
        k: model_cfg[k] for k in
        ("d_model", "n_layers", "n_heads", "d_ff", "dropout", "max_seq_len", "rope", "rmsnorm")
        if k in model_cfg
    })
