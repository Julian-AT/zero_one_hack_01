...
from src.data.load import make_online_loader, make_static_loader, load_all_families
from src.data.tokenizer import build_tokenizer
from src.model.registry import build_model
from src.train.losses import LossWeights, compute_losses
from src.train.tracking import Tracker, TrackerConfig
...
def cosine_lr(step: int, max_steps: int, warmup_steps: int, base_lr: float,
...
def train(cfg: dict[str, Any]) -> dict[str, float]:
    ...
    val_loader = make_static_loader(val_subset, tokenizer,
                                      batch_size=cfg["train"]["batch_size"],
                                      max_len=cfg["train"]["max_len"],
    ...
    while step < cfg["train"]["max_steps"]:
        # LR schedule
        lr = cosine_lr(step, cfg["train"]["max_steps"], cfg["train"]["warmup_steps"],
                        cfg["train"]["lr"])
        ...
        if step % cfg["train"]["log_every"] == 0:
            ...
            logger.info(f"step={step}  loss={log_dict['total_loss']:.4f}  "
        ...
        if step > 0 and step % cfg["train"]["eval_every"] == 0:
            ...
            logger.info(f"step={step}  val: " + " ".join(f"{k}={v:.4f}"
...
@torch.no_grad()
def evaluate(model: nn.Module, loader: DataLoader, weights: LossWeights,
              device: torch.device, amp_dtype: torch.dtype, use_amp: bool
              ) -> dict[str, float]:
    model.eval()
    sums: dict[str, float] = {}
    n_batches = 0
