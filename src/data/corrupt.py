...
from generate_sequences import (  # noqa: E402
    CLEAN_STEPS, DEPOSITION_STEPS, ETCH_STEPS, IMPLANT_STEPS, METAL_ETCH_STEPS,
    CMP_STEPS, BACKSIDE_METAL_STEPS, ELECTRICAL_TEST_STEPS, PAD_WINDOW_STEPS,
)
...
def corrupt_dep_no_clean(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    for di in dep_indices:
        ...
        for j in range(di - 1, max(-1, di - 13), -1):
            if steps[j] in CLEAN_STEPS:
                ...
                return Corruption("RULE_DEP_NO_CLEAN", new, j,
                                  f"removed clean step at index {j} ({steps[j]!r}) "
                                  f"before deposition at index {di} ({steps[di]!r})")
    return None


def corrupt_metal_etch_no_litho(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    for mi in me_indices:
        for j in range(mi - 1, max(-1, mi - 16), -1):
            if steps[j].startswith("EXPOSE LITHO LEVEL"):
                new = steps[:j] + steps[j + 1:]
                return Corruption("RULE_METAL_ETCH_NO_LITHO", new, j,
                                  f"removed EXPOSE LITHO at index {j} before metal "
                                  f"etch at index {mi}")
    return None


def corrupt_etch_no_mask(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    for ei in et_indices:
        for j in range(ei - 1, max(-1, ei - 13), -1):
            if steps[j] in ("DEVELOP PHOTORESIST", "DEVELOP PAD WINDOW"):
                new = steps[:j] + steps[j + 1:]
                return Corruption("RULE_ETCH_NO_MASK", new, j,
                                  f"removed DEVELOP at index {j} before etch at index {ei}")
    return None


def corrupt_litho_level_skip(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    return Corruption("RULE_LITHO_LEVEL_SKIP", new, pick,
                      f"removed {steps[pick]!r} at index {pick}")


def corrupt_implant_no_mask(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    openers = {"OXIDE ETCH", "OXIDE ETCH DRY", "ETCH SILICON OR OXIDE WINDOW",
               "DEVELOP PHOTORESIST"}
    for ii in imp_indices:
        for j in range(ii - 1, max(-1, ii - 16), -1):
            if steps[j] in openers:
                new = steps[:j] + steps[j + 1:]
                return Corruption("RULE_IMPLANT_NO_MASK", new, j,
                                  f"removed opener at index {j} ({steps[j]!r}) "
                                  f"before implant at index {ii}")
    return None


def corrupt_cmp_no_dep(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    for ci in cmp_indices:
        for j in range(ci - 1, max(-1, ci - 7), -1):
            if steps[j] in fill_set:
                new = steps[:j] + steps[j + 1:]
                return Corruption("RULE_CMP_NO_DEP", new, j,
                                  f"removed deposition/fill at index {j} ({steps[j]!r}) "
                                  f"before CMP at index {ci}")
    return None


def corrupt_pad_open_before_dep(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    return Corruption("RULE_PAD_OPEN_BEFORE_DEP", new, insert_at,
                      f"moved {pad_step!r} from index {pi} to {insert_at}")


def corrupt_test_before_passivation(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    return Corruption("RULE_TEST_BEFORE_PASSIVATION", new, insert_at,
                      f"moved test {test_step!r} from index {ti} to {insert_at} "
                      f"(before CURE PASSIVATION at {cure_idx})")


def corrupt_ship_before_test(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    return Corruption("RULE_SHIP_BEFORE_TEST", new, sort_idx - 1,
                      f"moved SHIP LOT from {ship_idx} to before WAFER SORT TEST")


def corrupt_backside_before_passivation(steps: list[str], rng: random.Random) -> Optional[Corruption]:
    ...
    return Corruption("RULE_BACKSIDE_BEFORE_PASSIVATION", new, insert_at,
...
def corrupt_random(steps: list[str], rng: random.Random,
                    verify: bool = True) -> Optional[Corruption]:
...
if __name__ == "__main__":
    ...
    from src.data.validator import generate_sequence
    rng = random.Random(0)
    seed_seq = generate_sequence("mosfet", rng)
    print(f"Baseline sequence: {len(seed_seq)} steps, valid={len(validate_sequence(seed_seq))==0}")
    print()
