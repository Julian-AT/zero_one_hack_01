...
def eval_nextstep_and_completion(
    lm, examples, frac: float, grammar: bool, canonicalize: bool, max_examples: int = 200
) -> dict:
    n = c1 = c3 = c5 = 0
    rr_sum = 0.0
    em = 0
    ...
    for ex in sample:
        ...
        # Completion
        gold = s[cut:]
        pred = complete_sequence(lm, ex.family, prefix, max_len=len(gold) + 20, grammar=grammar)
        if canonicalize:
...
def eval_anomaly(lm, examples, corrupt_frac: float = 0.5, max_examples: int = 200) -> dict:
    ...
    ] = []  # (seq, gold_is_valid, gold_rule, family)
    for ex in sample:
...
def main() -> None:
    ...
    for fam, fracs in results["per_family"].items():
        for frac, m in fracs.items():
            md.append(
                f"| {fam.upper()} | {frac} | {m['top1_at_cut']:.4f} | "
    ...
    md.append(
        f"- precision(valid) = {a['precision_valid']:.4f}, recall(valid) = {a['recall_valid']:.4f}"
    )
