...
```bash
# from the repo root
PYTHONPATH=neurosymbolic-approach python3 neurosymbolic-approach/experiments/exp01_symbolic_anomaly.py   # Task 3 oracle
PYTHONPATH=neurosymbolic-approach python3 neurosymbolic-approach/experiments/exp02_ppm_ranker.py          # Tasks 1&2, PPM
```
...
bash  neurosymbolic-approach/slurm/env_setup.sh        # login node: clone + pixi install + mkdir $SCRATCH/nspe_outputs
sbatch neurosymbolic-approach/slurm/test_debug.sbatch  # 1 GPU, 30 min: probe + exp01 + exp03 --smoke
sbatch neurosymbolic-approach/slurm/grid_lofo.sbatch   # 4 GPUs: LoFO holdout mosfet/igbt/ic + exp04, then aggregate
sbatch neurosymbolic-approach/slurm/train_full.sbatch  # 1 GPU: full train-all-3 + exp06 -> 3 submission CSVs
```
