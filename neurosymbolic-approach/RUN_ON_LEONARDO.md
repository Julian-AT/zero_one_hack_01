...
> CPU and live in `neurosymbolic-approach/outputs/charts/`. The Leonardo runs add
...
bash neurosymbolic-approach/slurm/env_setup.sh
...
```bash
sbatch neurosymbolic-approach/slurm/grid_lofo.sbatch   # 4×A100: LoFO mosfet/igbt/ic + constraint-loss ablation + aggregate + charts
sbatch neurosymbolic-approach/slurm/train_full.sbatch  # 1×A100: full 3-family model -> submissions + charts
squeue --me
...
scp "$H:\$SCRATCH/nspe_outputs/charts/*.png"          neurosymbolic-approach/outputs/charts/
scp "$H:\$SCRATCH/nspe_outputs/submission_task*.csv"  neurosymbolic-approach/outputs/
scp "$H:\$SCRATCH/nspe_outputs/*.json"                neurosymbolic-approach/outputs/
```
