...
.venv/bin/python scripts/demo_compare.py --example mosfet-mid

# Provide a custom prefix via pipe-separated steps
.venv/bin/python scripts/demo_compare.py \\
    --family mosfet \\
    --prefix "RECEIVE WAFER LOT|LOT IDENTIFICATION|INITIAL WAFER INSPECTION"

# Test anomaly detection on a corrupted sequence
.venv/bin/python scripts/demo_compare.py --example anomaly-mosfet

# Point at a specific checkpoint (default uses the best v2 final)
.venv/bin/python scripts/demo_compare.py --example mosfet-mid \\
    --lm-checkpoint extras/checkpoints/v2-final-transformer-small-lm_only-all3/final.pt \\
    --mt-checkpoint extras/checkpoints/v2-final-transformer-small-multitask-all3/final.pt
"""
...
_TRIGRAM_PATH = REPO_ROOT / "extras" / "baselines" / "trigram_baseline.py"
...
def _default_checkpoint(suffix: str) -> Path | None:
    ...
    candidates = [
        REPO_ROOT / "extras" / "checkpoints" / f"v3-final-transformer-medium-{suffix}-ood25-all3",
        REPO_ROOT / "extras" / "checkpoints" / f"v2-final-transformer-medium-{suffix}-all3",
        REPO_ROOT / "extras" / "checkpoints" / f"v2-final-transformer-small-{suffix}-all3",
        REPO_ROOT / "extras" / "checkpoints" / f"final-transformer-medium-{suffix}-fdp00-all3",
        REPO_ROOT / "extras" / "checkpoints" / f"final-transformer-small-{suffix}-fdp00-all3",
    ]
    for c in candidates:
