...
CATEGORIES = {
    ...
    "INSPECTION_GEOM": {"INITIAL WAFER INSPECTION", "PRE CLEAN INSPECTION",
                        "MEASURE THICKNESS", "MEASURE INITIAL THICKNESS",
                        "MEASURE INITIAL GEOMETRY", "MEASURE GEOMETRY",
                        "MEASURE SURFACE PARTICLES", "MEASURE SURFACE DEFECTS",
                        "MEASURE BACKSIDE ROUGHNESS",
                        "FINAL CLEAN", "FINAL THICKNESS MEASURE",
                        "FINAL GEOMETRY CHECK", "FINAL PARTICLE INSPECTION",
                        "FINAL OXIDE CHECK", "FINAL CD INSPECTION"},
    "CLEAN": {"PRE CLEAN WAFER", "WAFER CLEAN PRE PROCESS", "BACKSIDE CLEAN",
              "FRONTSIDE CLEAN", "WET CLEAN RCA1", "RCA CLEAN 1",
              "WET CLEAN RCA2", "RCA CLEAN 2", "HF DIP", "DRY WAFER",
              "DRY WAFER BACKSIDE", "CLEAN AFTER ETCH",
              "CLEAN AFTER OXIDE ETCH", "CLEAN AFTER WINDOW ETCH",
              "CLEAN AFTER FIELD ETCH", "CLEAN AFTER VIA ETCH",
              "CLEAN AFTER METAL ETCH", "CLEAN AFTER POLY ETCH",
              "CLEAN PAD OPENING", "BACKSIDE ETCH CLEAN", "BACKSIDE RINSE",
              "FRONTSIDE CLEAN FINAL", "BACKSIDE CLEAN FINAL",
              "WAFER CLEAN PRE-GRIND", "WAFER SURFACE CLEAN", "OXIDE STRIP",
              "SURFACE PREP FOR DEPOSITION"},
    "DEPOSITION": {"THERMAL OXIDATION", "GATE OXIDE PREP", "GATE OXIDE GROWTH",
                   "DEPOSIT PAD OXIDE", "ANNEAL OXIDE", "DEPOSIT POLYSILICON",
                   ...
                   "DEPOSIT INTERLEVEL DIELECTRIC", "DENSIFY DIELECTRIC",
                   "DENSIFY OXIDE", "DEPOSIT BARRIER METAL",
                   "DEPOSIT METAL SEED", "DEPOSIT METAL 1", "DEPOSIT TOP METAL",
                   "DEPOSIT BACKSIDE METAL", "DEPOSIT TUNGSTEN SEED",
                   "DEPOSIT PASSIVATION", "DEPOSIT PASSIVATION LAYER",
                   "DEPOSIT BACKSIDE PROTECTION", "EPITAXIAL DEPOSITION"},
    "LITHO": {"SPIN COAT PHOTORESIST", "SOFT BAKE", "POST EXPOSE BAKE",
              "DEVELOP PHOTORESIST", "HARD BAKE", "DEVELOP PAD WINDOW",
              "OPEN PAD WINDOW", "OPEN BOND PAD WINDOW", "PAD WINDOW LITHO",
              "OPEN PAD WINDOW LITHO"},
    "ETCH": {"OXIDE ETCH", "OXIDE ETCH DRY", "POLYSILICON ETCH",
             "POLYSILICON ETCH DRY", "ANISOTROPIC ETCH SPACER",
             "ETCH SILICON OR OXIDE WINDOW", "FIELD OXIDE ETCH", "VIA ETCH",
             "VIA ETCH THROUGH DIELECTRIC", "DIELECTRIC ETCH VIA",
             "METAL ETCH", "METAL ETCH DRY", "PASSIVATION ETCH PAD OPENING",
             "PASSIVATION ETCH", "ETCH WET BACKSIDE"},
    "STRIP": {"STRIP PHOTORESIST", "STRIP RESIST"},
    "IMPLANT": {"IMPLANT WELL", "IMPLANT SOURCE DRAIN", "IMPLANT SOURCE REGION",
                "IMPLANT LDD", "IMPLANT P BODY", "IMPLANT N BUFFER",
                "IMPLANT CHANNEL STOP", "IMPLANT DRAIN / CATHODE REGION",
                "IMPLANT N-TYPE", "DRIVE IN DIFFUSION", "RAPID THERMAL ANNEAL",
                "LIGHT ANNEAL", "PRE ANNEAL CHECK", "EPITAXY ANNEAL"},
    "CMP_FILL": {"CMP DIELECTRIC", "CMP INTERLAYER DIELECTRIC", "CMP METAL",
                 "CMP VIA FILL", "FILL VIA METAL", "FILL VIA TUNGSTEN"},
    "TEST": {"PARAMETRIC TEST", "ELECTRICAL PARAMETRIC TEST",
             "FINAL ELECTRICAL TEST PREP", "THRESHOLD VOLTAGE TEST",
             "LEAKAGE TEST", "BREAKDOWN VOLTAGE TEST", "SWITCHING TEST",
             "WAFER SORT TEST", "YIELD ANALYSIS"},
    "BACKSIDE_PREP": {"GRINDING WAFER BACKSIDE", "BACKSIDE GRIND",
                      "BACKSIDE THINNING CHECK", "RINSE WET WAFER_EDGE",
                      "BACKSIDE METALLIZATION PREP", "BACKSIDE ANNEAL",
                      "BACKSIDE DRY"},
...
def categorize(step: str) -> str:
    ...
    if step.startswith("ALIGN MASK LEVEL") or step.startswith("EXPOSE LITHO LEVEL"):
        return "LITHO"
    if step.startswith("INSPECT PATTERN") or step.startswith("PATTERN INSPECTION") \
       or step.endswith("INSPECTION") or step.endswith("WINDOW INSPECTION"):
        return "LITHO"
...
def load_long(path: Path) -> dict[str, list[str]]:
    ...
    for sid, step in zip(df["SEQUENCE_ID"], df["STEP"]):
        out[sid].append(step)
    return dict(out)
...
def main() -> None:
    ...
    for fam, seqs in family_seqs.items():
        ...
        ax.hist(lens, bins=30, alpha=0.55, label=fam.upper(),
                color=palette[fam], edgecolor="black", linewidth=0.3)
    ...
    vocab_per_fam = {fam: Counter(s for seq in seqs.values() for s in seq)
                     for fam, seqs in family_seqs.items()}
    ...
    for ax, (fam, seqs) in zip(axes, family_seqs.items()):
        ...
        for cidx, cat in enumerate(cats):
            ax.bar(np.arange(bin_count), bins[:, cidx], bottom=bottom,
                   color=cat_colors[cidx], label=cat, width=1.0)
            bottom += bins[:, cidx]
    ...
    fig.tight_layout()
    fig.savefig(OUT / "04_category_over_position.png", dpi=150,
                bbox_inches="tight")
    plt.close(fig)
    ...
    for fam, seqs in family_seqs.items():
        ...
        entropy_curves[fam] = {"mean_entropy_bits": float(ent.mean()),
    ...
    sns.heatmap(cov, annot=True, fmt=".2f", cmap="rocket_r",
                xticklabels=[f.upper() for f in fam_keys],
                yticklabels=[f.upper() for f in fam_keys],
                cbar_kws={"label": "fraction of A's bigrams seen in B"}, ax=ax)
    ...
    plt.close(fig)
    summary["bigram_coverage"] = {f"{A}_to_{B}": float(cov[i, j])
                                   for i, A in enumerate(fam_keys)
                                   for j, B in enumerate(fam_keys)}
    ...
    sns.heatmap(M_norm, ax=ax, cmap="rocket_r",
                xticklabels=top30, yticklabels=top30,
    ...
    for fam, seqs in family_seqs.items():
        ...
        fivegrams_per_seq = []
        for s in all_seqs:
            fivegrams_per_seq.append(set(zip(s, s[1:], s[2:], s[3:], s[4:])))
        all_fg = set().union(*fivegrams_per_seq) if fivegrams_per_seq else set()
        dup_stats[fam]["unique_5grams"] = len(all_fg)
        dup_stats[fam]["mean_5grams_per_seq"] = float(np.mean([len(x) for x in fivegrams_per_seq])) if fivegrams_per_seq else 0
    ...
    tri = dd(Counter); bi = dd(Counter); uni = Counter()
    for fam, seqs in family_seqs.items():
        for s in seqs.values():
            for i in range(len(s)):
                uni[s[i]] += 1
                if i >= 1: bi[s[i-1]][s[i]] += 1
                if i >= 2: tri[(s[i-2], s[i-1])][s[i]] += 1
    correct = total = 0
    correct3 = correct5 = 0
    for fam, seqs in family_seqs.items():
        for s in seqs.values():
            for i in range(2, len(s)):
                ...
                if ranked and ranked[0] == gold: correct += 1
                if gold in ranked[:3]: correct3 += 1
                if gold in ranked[:5]: correct5 += 1
                total += 1
    summary["trigram_backoff_topk"] = {
        "top1": correct/total, "top3": correct3/total, "top5": correct5/total,
        "n_predictions": total,
    }
    ...
    md.append("| family | n | mean | std | min | p25 | p50 | p75 | max |\n|---|--:|--:|--:|--:|--:|--:|--:|--:|")
    for fam, st in length_stats.items():
        md.append(f"| {fam.upper()} | {st['count']} | {st['mean']:.1f} | {st['std']:.1f} | "
    ...
    md.append(f"- Shared across all 3 families: **{summary['vocab']['shared_all_three']}** step strings")
    for fam in ["mosfet", "igbt", "ic"]:
        md.append(f"- {fam.upper()}-only steps ({len(summary['vocab']['exclusive'][fam+'_only'])}): "
    ...
    md.append(f"- Top-1: **{tk['top1']:.3f}**  |  Top-3: **{tk['top3']:.3f}**  |  Top-5: **{tk['top5']:.3f}**  "
              f"(n={tk['n_predictions']:,} predictions)")
    md.append("")
    md.append("**Position-conditional entropy:**")
    for fam, e in summary["position_entropy"].items():
        md.append(f"- {fam.upper()}: mean H = {e['mean_entropy_bits']:.2f} bits, "
    ...
    md.append("| family | n_seqs | exact dup | unique 5-grams | 5-grams/seq |\n|---|--:|--:|--:|--:|")
    for fam, d in summary["duplicate_stats"].items():
        md.append(f"| {fam.upper()} | {d['n_sequences']} | {d['exact_duplicate_sequences']} | "
