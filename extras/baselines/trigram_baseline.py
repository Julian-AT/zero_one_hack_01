...
Outputs are written to extras/results/baselines/ for the report.

Usage
-----
    python extras/baselines/trigram_baseline.py
    python extras/baselines/trigram_baseline.py --seed 7
"""
...
def normalized_edit_distance(a: list[str], b: list[str]) -> float:
    """Length-normalized Levenshtein distance between two step lists."""
    la, lb = len(a), len(b)
    if la == 0 and lb == 0:
        return 0.0
    if la == 0 or lb == 0:
...
def family_split(family_seqs: dict[str, list[list[str]]], seed: int = 42,
                 test_fraction: float = 0.2) -> tuple[dict, dict]:
...
def lofo_split(family_seqs: dict[str, list[list[str]]], held_out: str
...
def load_family_sequences() -> dict[str, list[list[str]]]:
    ...
    for fam, path in FAMILY_FILES.items():
        ...
        print(f"  loaded {fam}: {len(out[fam])} sequences, "
              f"mean len {sum(len(s) for s in out[fam]) / len(out[fam]):.1f}")
    return out


def write_topk_submission_csv(
    model: TrigramBackoff, eval_examples: list[tuple[str, list[str]]],
    out_path: Path,
) -> None:
...
def write_completion_submission_csv(
    model: TrigramBackoff, eval_examples: list[tuple[str, list[str], int]],
    out_path: Path,
) -> None:
...
def main() -> None:
    ...
    # ---- 1. EDA reproduction: train on ALL, eval on ALL (memorization upper bound)
    ...
    print(f"    n={m['n']:,}  Top-1={m['top1']:.4f}  Top-3={m['top3']:.4f}  "
          f"Top-5={m['top5']:.4f}  MRR={m['mrr']:.4f}")
    results["eda_reproduction"] = m

    # ---- 2. Honest ID split: train 80%, eval 20% per family
    print(f"\n[2] In-distribution held-out ({int(args.test_fraction*100)}% per family)")
    train_id, test_id = family_split(family_seqs, seed=args.seed,
    ...
    for fam, seqs in test_id.items():
        ...
        print(f"      {fam.upper():>6}: n={m['n']:,}  Top-1={m['top1']:.4f}  "
              f"Top-5={m['top5']:.4f}  MRR={m['mrr']:.4f}")
        results["id_holdout_per_family"][fam] = m

    # ---- 3. Truncation metrics (60% and 80%) — simulates eval_input_valid.csv
    ...
    # ---- 4. LoFO: train on 2 families, eval on 3rd  (OOD proxy for Task 4)
    ...
    for held_out in family_seqs:
        ...
        print(f"    held_out={held_out.upper():>6}  "
    ...
    # ---- 5. Emit a sample submission-format CSV using the held-out test set
    ...
    write_topk_submission_csv(model_id, eval_nextstep,
                              OUT_DIR / "trigram_nextstep_sample.csv")
    write_completion_submission_csv(model_id, eval_completion,
                                    OUT_DIR / "trigram_completion_sample.csv")

    # ---- 6. Save the full results JSON for the report
    ...
    # ---- 7. Markdown summary
    md = ["# Trigram-with-backoff baseline\n",
          f"Seed: {args.seed}.  Test fraction: {args.test_fraction}.\n",
          "## EDA reproduction (memorization upper bound)\n",
          f"Train and eval on all 3000 sequences. Reproduces the EDA number.\n",
    ...
    for fam, m in results["id_holdout_per_family"].items():
        md.append(f"| {fam.upper()} | {m['top1']:.4f} | {m['top3']:.4f} | "
                  f"{m['top5']:.4f} | {m['mrr']:.4f} |")
    md.append("\n## LoFO (Task 4 OOD proxy)\n")
    md.append("Train on 2 families, eval on the held-out 3rd. This is the number "
