...
def write_svg_bar(path, title, items, max_items=25, width=1100, bar_h=24):
    ...
    lines.append('<text x="20" y="32" font-family="Arial" font-size="22" font-weight="bold">%s</text>' % esc(title))

    for i, (label, value) in enumerate(items):
        y = top + i * bar_h
        bar_w = int(plot_w * value / max_val)
        lines.append('<text x="10" y="%d" font-family="Arial" font-size="12">%s</text>' % (y + 16, esc(str(label)[:45])))
        lines.append('<rect x="%d" y="%d" width="%d" height="16" fill="#4C78A8"/>' % (left, y + 3, bar_w))
        lines.append('<text x="%d" y="%d" font-family="Arial" font-size="12">%s</text>' % (left + bar_w + 8, y + 16, esc(value)))
...
def write_svg_hist(path, title, values, bin_size=5):
    ...
    write_svg_bar(path, title, sorted(bins.items(), key=lambda x: int(x[0].split("-")[0])), max_items=50)
...
def main():
    ...
    for row in completion_rows:
        steps = split_steps(row.get("PREDICTED_SEQUENCE", ""))
    ...
    report.append("The organizer eval inputs are unlabeled, so this report does **not** show official accuracy or F1. It only checks and visualizes our generated prediction files.\n")

    report.append("## File Counts\n")
    report.append(write_md_table(
    ...
    report.append(write_md_table(
        ...
        [
            ...
            ["Min predicted suffix length", min(completion_lengths) if completion_lengths else 0],
            ["Max predicted suffix length", max(completion_lengths) if completion_lengths else 0],
        ],
    ))
    ...
    report.append("- The anomaly predictions are validator-based, so they reflect whether the explicit 10 process rules flag a sequence as invalid.")
