...
Multitask config trained for 8 k steps initially. From the TB plot `shared/extras/plots/training/heads_loss.png`, both `validity_loss` and `rule_id_loss` flatten at step 4000. Phase-2 cut `max_steps` to 6000 → 30 % wall savings, no quality loss.
...
All plots are PNG, regenerated by `shared/scripts/plot_training.py` and `shared/scripts/plot_submission_quality.py`:

| Path | What it shows |
|---|---|
| `shared/extras/plots/training/train_lm_loss_by_arch.png` | Training LM-loss curves, faceted arch × size, colored by held-out family |
| `shared/extras/plots/training/val_lm_loss_by_arch.png` | Same for val LM loss |
| `shared/extras/plots/training/heads_loss.png` | Multitask validity-BCE + rule-ID-CE curves |
| `shared/extras/plots/training/throughput.png` | Median steps/sec per cell, transformer vs xLSTM |
| `shared/extras/plots/training/scaling_curve.png` | Final LM loss vs param count |
| `shared/extras/plots/training/per_fold_overlay.png` | Val curves per held-out family |
| `shared/extras/plots/report/trajectory.png` | OOD Top-1 trajectory: trigram → Phase-1 → Phase-2 → Phase-3 |
| `shared/extras/plots/report/max_len_fix.png` | Single-cell A/B: max_len=256 vs 768 |
| `shared/extras/plots/report/phase_comparison.png` | Per-fold Top-1_held across all phases |
| `shared/extras/plots/report/submission_quality.png` | 100 % validator-clean / perfect class balance / 10-rule distribution |
| `shared/extras/plots/report/scaling_corrected.png` | Bigger ≠ better + Phase-2 stars below the floor |

11 plots total. EDA plots in `shared/extras/eda/` are separate (7 PNGs) and predate the training phases.
